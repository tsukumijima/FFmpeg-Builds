...
cat <<EOF >"$BUILD_SCRIPT"
    ...
    ./configure --prefix=/ffbuild/prefix --pkg-config-flags="--static" \$FFBUILD_TARGET_FLAGS $FF_CONFIGURE \
        --enable-hwaccels --enable-v4l2_m2m --enable-sand --disable-mmal --disable-rpi \
...
docker run --rm -i $TTY_ARG "${UIDARGS[@]}" -v $PWD/ffbuild:/ffbuild -v "$PWD/patches/ffmpeg/rpi":/patches -v "$BUILD_SCRIPT":/build.sh "$IMAGE" bash /build.sh
